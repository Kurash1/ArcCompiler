            ...
            for(int i = 0; i < compileFile.Length; i++)
            {
                switch (compileFile[i])
                {
                    case "namespace": //Namespace
                        if (compileFile[i+1] == "=")
                        {
                            WriteEvent(compileFile[i + 0]);
                            NameSpace = compileFile[i + 2];
                            i += 2;
                        }
                        else
                        {
                            Console.WriteLine("Error: Expecting \"=\" Word: " + i);
                            return;
                        }
                        break;
                    case "continent": case "add_base_tax": //<something> = <something>
                        if (compileFile[i+1] == "=")
                        {
                            WriteEvent(compileFile[i + 0]);
                            WriteEvent(compileFile[i + 1]);
                            WriteEvent(compileFile[i + 2]);
                        }
                        else
                        {
                            Console.WriteLine("Error: Expecting \"=\" Word: " + i);
